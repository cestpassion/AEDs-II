...
void insercao(Pokemon *, int, int *, int *);
...
void heapsort(Pokemon *, int, int *, int *);
void construir(Pokemon *, int);
void reconstruir(Pokemon *, int);
int getMaiorHeight(Pokemon *, int, int);
...
// Potótipos das funções de Ordenação PARCIAL
void insercaoParcial(Pokemon *, int, int *, int *);
void heapsortParcial(Pokemon *, int, int *, int *);
...
int main()
{
    ...
    heapsortParcial(pokemonArray, tam, &comparacoes, &movimentacoes);
    ...
    FILE *log = fopen("files/847503_heapsortPARCIAL.txt", "w");
    if (log == NULL)
    {
...
void insercao(Pokemon *pokemonArray, int tam, int *comparacoes, int *movimentacoes)
{
    for (int i = 1; i < tam; i++)
    {
        ...
        (*comparacoes)++;
        while ((j >= 0) && compareDates(pokemonArray[j].date, tmp.date) > 0)
        {
            (*movimentacoes)++;
...
{
    // Criar as variáveis para armazenar o formato invertido
    char formattedDate1[9], formattedDate2[9];

    // Converter para o formato "YYYYMMDD"
    snprintf(formattedDate1, 9, "%c%c%c%c%c%c%c%c", date1[6], date1[7], date1[8], date1[9], date1[3], date1[4], date1[0], date1[1]);
    snprintf(formattedDate2, 9, "%c%c%c%c%c%c%c%c", date2[6], date2[7], date2[8], date2[9], date2[3], date2[4], date2[0], date2[1]);

    // Comparar as datas no formato "YYYYMMDD"
...
void heapsort(Pokemon *pokemonArray, int tam, int *comparacoes, int *movimentacoes)
{
    // Alterar o vetor ignorando a posicao zero
    Pokemon tmp[tam + 1];
    for (int i = 0; i < tam; i++)
    {
        tmp[i + 1] = pokemonArray[i];
    }
    ...
    for (int i = 0; i < tam; i++)
    {
        pokemonArray[i] = tmp[i + 1];
    }
}

void construir(Pokemon *pokemonArray, int tamHeap)
{
    for (int i = tamHeap; i > 1 && (pokemonArray[i].height > pokemonArray[i / 2].height ||
                                    (pokemonArray[i].height == pokemonArray[i / 2].height &&
                                    strcmp(pokemonArray[i].name, pokemonArray[i/2].name) > 0));
                                    i /= 2)
    {
        swap(pokemonArray, i, i / 2);
    }
}

void reconstruir(Pokemon *pokemonArray, int tamHeap)
{
    int i = 1;
    while (i <= (tamHeap / 2))
    {
        int filho = getMaiorHeight(pokemonArray, i, tamHeap);

        if (pokemonArray[i].height < pokemonArray[filho].height ||
            (pokemonArray[i].height == pokemonArray[filho].height &&
             strcmp(pokemonArray[i].name, pokemonArray[filho].name) < 0))
        {
            swap(pokemonArray, i, filho);
            i = filho;
        }
        else
        {
...
int getMaiorHeight(Pokemon *pokemonArray, int i, int tamHeap)
{
    int filho;
    ...
    else if (pokemonArray[2 * i].height > pokemonArray[2 * i + 1].height ||
             (pokemonArray[2 * i].height == pokemonArray[2 * i + 1].height &&
              strcmp(pokemonArray[2 * i].name, pokemonArray[2 * i + 1].name) > 0))
    {
...
void insercaoParcial(Pokemon *pokemonArray, int tam, int *comparacoes, int *movimentacoes)
{
    for (int i = 1; i < tam; i++)
    {
        Pokemon tmp = pokemonArray[i];
...
void heapsortParcial(Pokemon *pokemonArray, int tam, int *comparacoes, int *movimentacoes)
{
    // Alterar o vetor ignorando a posicao zero
    Pokemon tmp[tam + 1];
    for (int i = 0; i < tam; i++)
    ...
    for(int i = k + 1; i <= tam; i++){
        if(pokemonArray[i].height < pokemonArray[1].height){
            swap(tmp, i, 1);
            reconstruir(tmp, k);
        }
    }
