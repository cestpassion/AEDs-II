...
public void countingsort() {
    // Usando um atributo numérico para contagem (por exemplo, captureRate)
    int max = getMaior();  // Considerando que a captureRate é o critério de ordenação
    
    // Inicializar o array de contagem com tamanho adequado para os valores de captureRate
    int[] count = new int[max + 1];  // O índice vai de 0 até max
    ...
    // Contar as ocorrências de cada captureRate
    for (int i = 0; i < n; i++) {
        int index = array[i].getCaptureRate();  // CaptureRate já é um int, sem necessidade de multiplicação
        count[index]++;
    }
    
    // Atualizar o array de contagem para armazenar as posições acumuladas
    ...
    for (int i = n - 1; i >= 0; i--) {
        int index = array[i].getCaptureRate();  // CaptureRate já é um int
        ordenado[count[index] - 1] = array[i];
        count[index]--;
    }
    ...
    for (int i = (n - 1); i > 0; i--) {
        for (int j = 0; j < i; j++) {
            if (array[j].getCaptureRate() == array[j + 1].getCaptureRate()) {
...
// Método para encontrar o maior valor de captureRate
public int getMaior() {
    int maior = array[0].getCaptureRate();  // Começa com o valor da captureRate do primeiro Pokémon
    
    for (int i = 1; i < n; i++) {
        if (array[i].getCaptureRate() > maior) {
            maior = array[i].getCaptureRate(); // Atualiza o maior valor
        }
    }
    return maior;
}
...
public class CountingSort {
    public static void main(String[] args){
        ...
        Ordenacao countingsort = new Ordenacao(pokemonArray, index);
        ...
        int comparacoes = countingsort.getComparacoes();
        int movimentacoes = countingsort.getMovimentacoes(); 

        pokemonArray = countingsort.getArray();
        for(int i = 0; i < index; i++)
            pokemonArray[i].imprimir();
        
        // Criar o arquivo de log
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("files/847503_countingsort.txt"))) {
